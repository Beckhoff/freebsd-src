    ...
    const long arg2 __unused)
{
	...
	struct tpm_ppi_qemu *const ppi_region = dev->ppi_data;

	const uint64_t off = addr - TPM_PPI_ADDRESS;
	uint8_t *const ptr = (uint8_t *)ppi_region + off;
...
static int
tpm_ppi_init(struct tpm_device *dev)
{
	int error;

	struct tpm_ppi *const ppi = dev->ppi;

	struct tpm_ppi_qemu *const ppi_qemu = calloc(1, TPM_PPI_SIZE);
	if (ppi_qemu == NULL) {
		warnx("%s: failed to allocate acpi region for ppi\n", __func__);
		return (ENOMEM);
	}

	dev->ppi_data = ppi_qemu;

	struct tpm_ppi_fwcfg *fwcfg = calloc(1, sizeof(struct tpm_ppi_fwcfg));
	if (fwcfg == NULL) {
		warnx("%s: failed to allocate fwcfg item\n", __func__);
		return (ENOMEM);
	}
	...
	snprintf(mr_name, sizeof(mr_name), "tpm_ppi_%s", ppi->name);
	mr.base = TPM_PPI_ADDRESS;
	mr.size = TPM_PPI_SIZE;
	mr.flags = MEM_F_RW;
	mr.handler = tpm_ppi_mem_handler;
	mr.arg1 = dev;
	error = register_mem(&mr);
	if (error) {
		warnx("%s: failed to create trap for ppi accesses\n", __func__);
		return (error);
	}
...
static void
tpm_ppi_deinit(struct tpm_device *dev)
{
	if (dev == NULL) {
		return;
	}
...
static int
tpm_ppi_write_dsdt_regions(const struct tpm_device *const dev __unused)
{
	/*
	 * struct tpm_ppi_qemu
	 */
...
static int
tpm_ppi_write_dsdt_dsm(const struct tpm_device *const dev __unused)
{
	/*
