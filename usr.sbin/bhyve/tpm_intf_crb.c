...
{
	struct tpm_device *const dev = arg;
	...
	for (;;) {
		...
		if (dev->emul->execute_cmd) {
			dev->emul->execute_cmd(dev,
			    &crb->regs.data_buffer[cmd_off], cmd_size,
...
static int
tpm_crb_init(struct tpm_device *dev)
{
	int error;

	struct tpm_intf *const intf = dev->intf;

	struct tpm_crb *const crb = calloc(1, sizeof(struct tpm_crb));
	dev->intf_data = crb;
	if (crb == NULL) {
		warnx("%s: failed to allocate tpm crb\n", __func__);
		return (ENOMEM);
	}
	...
	if (error) {
		warnx("%s: failed to init cond\n", __func__);
		return (error);
	}

	error = pthread_create(&crb->thread, NULL, tpm_crb_thread, dev);
	if (error) {
...
static void
tpm_crb_deinit(struct tpm_device *dev)
{
	if (dev == NULL) {
		return;
	}
...
static int
tpm_crb_build_acpi_table(const struct tpm_device *const dev)
{
	struct vmctx *const ctx = dev->ctx;

	struct basl_table *table;
	BASL_EXEC(basl_table_create(&table, ctx, ACPI_SIG_TPM2,
